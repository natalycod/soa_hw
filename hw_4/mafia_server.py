...
MIN_USERS_COUNT = 4
MAFIA_COEF = 4
REST_HOST = 'http://rest_server:5000'
...
class User:
    def __init__(self, session_name, name):
        print("creating user")
    ...
    def add_chat_message(self, message):
        ...
        write_channel.queue_declare(queue=self.session_name + "#" + self.name)
        write_channel.basic_publish(exchange='', routing_key=self.session_name + "#" + self.name, body=message.text)
...
class Session:
    ...
    def _start_queue_connection(self):
        connection = pika.BlockingConnection(pika.ConnectionParameters('0.0.0.0'))
        channel = connection.channel()
        channel.queue_declare(queue="session")
        channel.basic_consume(queue="session", auto_ack=True, on_message_callback=_callback_session_queue)
        channel.start_consuming()
    ...
    def add_user(self, user_name):
        self.users[user_name] = User(self.session_name, user_name)
        for name, user in self.users.items():
    ...
    def send_chat_message(self, user_name, text):
        ...
        for name in receivers:
            ...
            self.users[name].add_chat_message(message)
...
class MafiaConnection(mafia_pb2_grpc.MafiaServicer):
    def ConnectToServer(self, request, context):
        if request.session_name not in current_sessions:
            current_sessions[request.session_name] = Session(request.session_name, max(request.users_count, MIN_USERS_COUNT))
            threading.Thread(target=create_session_queue, daemon=False, args=[request.session_name]).start()
