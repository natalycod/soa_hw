...
def _handle_queue_chat_message(ch, method, properties, body):
    print(bcolors.BOLD + bcolors.OKGREEN + body.decode() + bcolors.ENDC + bcolors.ENDC)
...
class UserSession:
    def __init__(self, executor, channel, session_name, user_name, users_count):
        ...
        threading.Thread(target=self.read_channel.start_consuming(), daemon=False, args=[]).start()
...
def register(user_name):
    ...
    if resp.json()["status"] == "not_found":
        print("User with this name doesn't exist. Please fill some information below")
        print("Your sex (male/female): ", end="")
        sex = input()
        print("Your email: ", end="")
        email = input()
        print("Path to your user image: ", end="")
        image = input()
        req = {"name": user_name, "sex": sex, "email": email, "image": image}
        requests.post(HOST + "/add_user", json=req)
